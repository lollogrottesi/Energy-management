...
#CSV variables.
outCsv_tis="${dst}_dpm_report_overhead_idle_sleep.csv"
outCsv_t="${dst}_dpm_report_overhead_idle.csv"
...
echo "distribution;timeoutIdle;timeOverhead;energyOverhead" >> ${outCsv_t}

echo "Parameters => idle_timeout: [${min_tm_idle}, ${max_tm_idle}]; sleep_timeout: [${min_tm_sleep}; ${max_tm_sleep}]; step: ${step_tm}" >> ${outCsv_tis}
echo "distribution;timeoutIdle;timeoutSleep;timeOverhead;energyOverhead" >> ${outCsv_tis}
...
for ((i = $min_tm_idle ; i <= $max_tm_idle ; i += step_tm)); do
	...
	echo "${dst};$i;${array_dpm_out_t[141]};${array_dpm_out_t[154]}" >> ${outCsv_t}
	...
	for ((j = $min_tm_sleep ; j <= $max_tm_sleep ; j += step_tm)); do
		...
		echo "${dst};$i;$j;${array_dpm_out[141]};${array_dpm_out[154]}" >> ${outCsv_tis}
	done
done
echo ""
